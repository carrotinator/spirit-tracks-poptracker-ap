...
spirit_tracks_location = {}
spirit_tracks_location.__index = spirit_tracks_location
...
function spirit_tracks_location.new(name)
    local self = setmetatable({}, spirit_tracks_location)
    if name then
...
function spirit_tracks_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = spirit_tracks_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
function spirit_tracks_location:connect_two_ways(exit, rule)
...
function spirit_tracks_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function spirit_tracks_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function spirit_tracks_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function spirit_tracks_location:connect_two_ways_stuck(exit, rule1, rule2)
...
function spirit_tracks_location:accessibility()
...
-- 
function spirit_tracks_location:discover(accessibility, keys)
...
entry_point = spirit_tracks_location.new("entry_point")
